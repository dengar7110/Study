...
FROM `realtor` JOIN `real_estate`
ON realtor.id = real_estate.realtorId
WHERE real_estate.type = '월세' AND realtor.grade = '안심중개사'
ORDER BY real_estate.rentPrice;
...
FROM `realtor` JOIN `real_estate` 
ON realtor.id = real_estate.realtorId
WHERE realtor.office = '대박공인중개사' 
	AND real_estate.type IN('매매', '전세')
-- 	AND (real_estate.type = '매매' OR real_estate.type = '전세') 
    AND real_estate.area >= 100;
